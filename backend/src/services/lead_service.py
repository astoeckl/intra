...
Business logic for lead management including form processing and bulk import.
"""
...
async def get_leads(
    ...
    """
    Retrieve paginated leads with optional filters.

    Returns leads with contact and campaign relationships eagerly loaded.
    """
    query = (
        select(Lead)
        .options(
...
async def create_lead_from_form(
    ...
    Automatically creates or reuses company and contact records.
    Contact deduplication is based on email address.
    """
    company_id = None
...
async def import_leads_from_file(
    ...
    Supports German (vorname, nachname, firma, telefon) and English
    column names. Creates companies/contacts as needed with email-based
    deduplication.

    Returns import statistics with first 10 errors if any.
    """
    errors: list[str] = []
    imported = 0
